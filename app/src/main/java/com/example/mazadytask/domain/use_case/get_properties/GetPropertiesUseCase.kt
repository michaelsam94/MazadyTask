package com.example.mazadytask.domain.use_case.get_properties
...
class GetPropertiesUseCase @Inject constructor(private val repository: MazadyRepository) {

    operator fun invoke(catId: Int): Flow<Resource<List<Property>>> = flow {
        try {
            emit(Resource.Loading())
            val categories = repository.getProperties(catId).data.map { it.toProperty() }
